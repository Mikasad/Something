...
void Net_Manager_Handle(CO_Data *d)
{
      ...
      )
    {
        ...
        if((d->CanRx_Buffer->Data[1] == d->Node_ID)||(d->CanRx_Buffer->Data[1] == 0))
        {
            ...
            {
                if(d->NMT_state == PRE_OPERATIONG || d->NMT_state == STOPSTOP)
                {
...
//            }
            ...
            {
                if(d->NMT_state == OPERATION_STATE || d->NMT_state == STOPSTOP)
                {
...
unsigned char SendBootUp(CO_Data *d)
{
    ...
    return CAN2_Send_Msg(d->CanTx_Buffer->StdId,CAN_ID_STD,CAN_RTR_DATA,d->CanTx_Buffer->Data,d->CanTx_Buffer->DLC);
}
/*------------------------------------------------
