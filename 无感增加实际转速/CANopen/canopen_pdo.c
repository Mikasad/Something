...
//			ATLAS_PRINT("sendPDOrequest cobId is : %d\r\n", *pwCobId);

            {
                ...
                /* 发送报文 */
                return CAN2_Send_Msg(pdo.StdId,CAN_ID_STD,pdo.RTR,pdo.Data,pdo.DLC);
            }
        }
    }
            ...
            while(offsetObjdict <= lastIndex)
            {
                ...
                case state4:
                {
                    ...
                    else if(*pTransmissionType == SYNC_RTR_ONLY)
                    {
                        ...
                        {
                            CAN2_Send_Msg(d->PDO_status[numPdo].last_message.StdId,CAN_ID_STD,d->PDO_status[numPdo].last_message.RTR,d->PDO_status[numPdo].last_message.Data,d->PDO_status[numPdo].last_message.DLC);
                            return 0;
                        }
                        else
                        {
...
//							ATLAS_PRINT(" Couldn't build TPDO number : %d\r\n", numPdo);
                    ...
                    CAN2_Send_Msg(pdo.StdId,CAN_ID_STD,pdo.RTR,pdo.Data,pdo.DLC);
                    return 0;
                }
                }
            }
