...
//  hSpeed_Reference = PID_SPEED_REFERENCE;
    ...
    PID_Speed->hLower_Limit_Output= -MotorControl[0].Current.MaxValue4;   //Lower Limit for Output limitation
    PID_Speed->hUpper_Limit_Output= MotorControl[0].Current.MaxValue4;   //Upper Limit for Output limitation
    PID_Speed->wLower_Limit_Integral = -MotorControl[0].Current.MaxValue4 * SP_KIDIV;
    PID_Speed->wUpper_Limit_Integral = MotorControl[0].Current.MaxValue4 * SP_KIDIV;
    PID_Speed->wIntegral = 0;
    PID_Speed->Ksat = 1;
    /**************************************************/
    /**********END PID Speed Regulator members*********/
...
s16 PID_Regulator(s16 hReference, s16 hPresentFeedback, PID_Struct_t *PID_Struct)
{
    ...
    if (PID_Struct->OutPreSat >= 7980)  //7980 = 8400*0.95
    {
        houtput_32=(PID_Struct->hUpper_Limit_Output);
    }
    else if (PID_Struct->OutPreSat< -7980)
    {
